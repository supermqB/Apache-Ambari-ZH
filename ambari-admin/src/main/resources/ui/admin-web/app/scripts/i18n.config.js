...
angular.module('ambariAdminConsole').config([
  '$translateProvider',
  function ($translateProvider) {
    $translateProvider.translations('en', {
      ...
      'common.deleteConfirmation':
        'Are you sure you want to delete {{instanceType}} {{instanceName}}?',
      'common.remoteClusterDelConfirmation':
        'Are you sure you want to delete {{instanceType}} {{instanceName}}? This operation cannot be undone.',
      'common.messageInstanceAffected':
        'The following View Instances are using this Remote Cluster for configuration, and will need to be reconfigured:',
      'common.local': 'Local',
      ...
      'common.clusterNameChangeConfirmation.title':
        'Confirm Cluster Name Change',
      'common.clusterNameChangeConfirmation.message':
        'Are you sure you want to change the cluster name to {{clusterName}}?',

      'common.loginActivities.loginActivities': 'Login Activities',
      'common.loginActivities.loginMessage': 'Login Message',
      'common.loginActivities.loginMessage.placeholder':
        'Please enter login message',
      ...
      'common.loginActivities.buttonText.placeholder':
        'Please enter button text',

      'common.loginActivities.status': 'Status',
      'common.loginActivities.status.disabled': 'Disabled',

      'common.loginActivities.homeDirectory.alert':
        "Many Ambari Views store user preferences in the logged in user's / user directory in HDFS. Optionally, Ambari can auto-create these directories for users on login.",
      'common.loginActivities.homeDirectory.autoCreate':
        'Auto-Create HDFS user directories',
      'common.loginActivities.homeDirectory.header':
        'User Directory Creation Options',
      'common.loginActivities.homeDirectory.template':
        'User Directory creation template',
      ...
      'common.alerts.noPrivilegesDescription':
        'This {{term}} does not have any privileges.',
      'common.alerts.timeOut':
        'You will be automatically logged out in <b>{{time}}</b> seconds due to inactivity.',
      ...
      'common.alerts.clusterRenamed':
        'The cluster has been renamed to {{clusterName}}.',
      'common.alerts.remoteClusterRegistered':
        'The cluster has been registered as {{clusterName}}.',
      'common.alerts.cannotRenameCluster':
        'Cannot rename cluster to {{clusterName}}',
      ...
      'common.alerts.onlyText':
        'Only lowercase alphanumeric characters are allowed.',
      'common.alerts.onlyAnScore':
        'Invalid input, only alphanumerics allowed eg: My_default_view',
      'common.alerts.passwordRequired': 'Password Required',
      'common.alerts.unsavedChanges':
        'You have unsaved changes. Save changes or discard?',
      'common.alerts.cannotUpdateStatus': 'Cannot update User status',
      'common.alerts.cannotUpdateAdminStatus':
        'Cannot update User Admin status',
      ...
      'main.createCluster.description':
      ...
      'views.clusterPermissions.infoMessage':
        'Grant <strong>Use</strong> permission for the following <strong>{{cluster}}</strong> Roles:',
      'views.clusterPermissions.nonLocalClusterMessage':
        'The ability to inherit view <strong>Use</strong> permission based on Cluster Roles is only available when using a Local Cluster configuration.',

      'views.alerts.noSpecialChars': 'Must not contain any special characters.',
      'views.alerts.noSpecialCharsOrSpaces':
      ...
      'views.alerts.cannotDeleteStaticInstance':
      ...
      'views.alerts.unableToResetErrorMessage':
        'Unable to reset error message for prop: {{key}}',
      'views.alerts.instanceCreated': 'Created View Instance {{instanceName}}',
      'views.alerts.unableToParseError':
        'Unable to parse error message: {{message}}',
      ...
      'views.alerts.savedRemoteClusterInformation':
        'Remote cluster information is saved.',
      ...
      'urls.change': 'Change',
      'urls.urlCreated':
        'Created short URL <a href="{{siteRoot}}#/main/view/{{viewName}}/{{shortUrl}}">{{urlName}}</a>',
      'urls.urlUpdated':
        'Updated short URL <a href="{{siteRoot}}#/main/view/{{viewName}}/{{shortUrl}}">{{urlName}}</a>',
      ...
      'groups.alerts.groupCreated':
        'Created group <a href="#/groups/{{groupName}}/edit">{{groupName}}</a>',
      ...
      'users.userNameTip':
        'Maximum length is 80 characters. \\, &, |, <, >, ` are not allowed.',
      'users.adminTip':
        'An Ambari Admin can create new clusters and other Ambari Admin Users.',
      'users.deactivateTip':
        'Active Users can log in to Ambari. Inactive Users cannot.',

      'users.changeStatusConfirmation.title': 'Change Status',
      'users.changeStatusConfirmation.message':
        'Are you sure you want to change status for user "{{userName}}" to {{status}}?',

      'users.changePrivilegeConfirmation.title': 'Change Admin Privilege',
      'users.changePrivilegeConfirmation.message':
        'Are you sure you want to {{action}} Admin privilege to user "{{userName}}"?',
      ...
      'users.roles.oneRolePerUserOrGroup':
        'Only 1 role allowed per user or group',
      ...
      'users.alerts.userCreated':
        'Created user <a href="#/users/{{encUserName}}/edit">{{userName}}</a>',
      ...
      'users.alerts.roleChangedToNone':
        "{{user_name}}'s explicit privilege has been changed to 'NONE'. Any privilege now seen for this user comes through its Group(s).",
      'users.alerts.usersEffectivePrivilege':
        "{{user_name}}'s effective privilege through its Group(s) is higher than your selected privilege.",
      ...
      'versions.networkIssues.publicDisabledHeader':
        'Public Repository Option Disabled',
      'versions.networkIssues.publicRepoDisabledMsg':
        'Ambari does not have access to the Internet and cannot use the Public Repository for installing the software. Your Options:',
      'versions.networkIssues.publicRepoDisabledMsg1':
        'Configure your hosts for access to the Internet.',
      'versions.networkIssues.publicRepoDisabledMsg2':
      ...
      'versions.networkIssues.publicRepoDisabledMsg3':
        'Use the Local Repositoy option.',
      ...
      'versions.register.error.body':
        'You are attempting to register a version with a Base URL that is already in use with an existing registered version. You *must* review your Base URLs and confirm they are unique for the version you are trying to register.',

      'versions.deregister': 'Deregister Version',
      'versions.deregisterConfirmation':
        'Are you sure you want to deregister version <strong>{{versionName}}</strong> ?',
      'versions.placeholder': 'Version Number {{pattern}}',
      'versions.repos': 'Repositories',
      'versions.os': 'OS',
      'versions.baseURL': 'Base URL',
      'versions.skipValidation':
      ...
      'versions.introduction':
        'To register a new version in Ambari, provide a Version Definition File, confirm the software repository information and save the version.',
      ...
      'versions.useRedhatSatellite.warning':
        'In order for Ambari to install packages from the right repositories, ' +
      ...
      'versions.useRedhatSatellite.disabledMsg':
        'Use of RedHat Satellite/Spacewalk is not available when using Public Repositories',

      'versions.changeBaseURLConfirmation.title': 'Confirm Base URL Change',
      'versions.changeBaseURLConfirmation.message':
        'You are about to change repository Base URLs that are already in use. Please confirm that you intend to make this change and that the new Base URLs point to the same exact Stack version and build',

      'versions.alerts.baseURLs':
        'Provide Base URLs for the Operating Systems you are configuring.',
      'versions.alerts.validationFailed':
      ...
      'versions.alerts.skipValidationWarning':
        'Warning: This is for advanced users only. Use this option if you want to skip validation for Repository Base URLs.',
      'versions.alerts.useRedhatSatelliteWarning':
        'Disable distributed repositories and use RedHat Satellite/Spacewalk channels instead',
      'versions.alerts.filterListError':
        'Fetch stack version filter list error',
      'versions.alerts.versionCreated':
        'Created version <a href="#/stackVersions/{{stackName}}/{{versionName}}/edit">{{stackName}}-{{versionName}}</a>',
      ...
      'versions.alerts.versionEdited':
        'Edited version <a href="#/stackVersions/{{stackName}}/{{versionName}}/edit">{{displayName}}</a>',
      'versions.alerts.versionUpdateError': 'Version update error',
      'versions.alerts.versionDeleteError': 'Version delete error',
      'versions.alerts.repositoryExists':
        'A repository with the same Repo ID already exists for {{os}}!',
      'versions.alerts.cannotDeleteInstalled':
        'Cannot delete version already installed.',

      'authentication.description':
        'Ambari supports authenticating against local Ambari users created and stored in the Ambari Database, or authenticating against a LDAP server:',
      ...
      'authentication.attributes.detection.label':
        'Identifying the proper attributes to be used when authenticating and looking up users and groups can be specified manually, or automatically detected. Please choose:',

      'authentication.attributes.detection.options.manual':
        'Define Attributes Manually',
      'authentication.attributes.detection.options.auto':
      ...
      'authentication.attributes.detected':
        'The following attributes were detected, please review and Test Attributes to ensure their accuracy.',
      ...
      'authentication.attributes.distinguishedNameAttr':
        'Distinguished Name Attribute',

      'authentication.attributes.test.description':
        'To quickly test the chosen attributes click the button below. During this process you can specify a test user name and password and Ambari will attempt to authenticate and retrieve group membership information',
      ...
      'authentication.attributes.alerts.successfulAuth':
